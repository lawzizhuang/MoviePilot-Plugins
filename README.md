...
- `/tv`、`/movie`支持每批1至10条混合115分享、ED2K、BTIH磁力，推荐每行一条，兼容框架折叠换行。链接内部空格须URL编码。
- 全批格式预检，串行复用限速和任务互斥；同批／历史去重，每条成功立即保存。明确过期分享跳过，部分成功、提交不确定、状态保存失败或熔断时停止后续条目。
- 保持现有接收路径与测试模式，不修改订阅、不重复触发整理。首次请先测试模式，再用两条资源验收。

### v2.4.16

- DMHY动漫RSS改为优先按MoviePilot已确认的主标题、原始标题进行作品级RSS检索，可覆盖滚动500条Feed之外的早期单集与整季包。
- 命中作品级结果后不再读取两个滚动Feed；未命中才回退滚动Feed。新增每轮关键词请求上限（默认6），同关键词内存缓存，继续受请求间隔与熔断保护。
- 首次启用建议测试模式，观察关键词预算、候选标题、季集及过滤器结果；不要因可检索历史资源而一次性补齐大批缺集。

### v2.4.15

- 新增默认关闭的DMHY动漫RSS补充源：仅AniList已识别动画TV，在既有Telegram、4K Monitor、SeedHub离线候选之后尝试。
- 每轮最多读取动画与季度全集RSS各一次，仅保留最小BTIH、移除RSS tracker；403、429、5xx和XML异常熔断当轮。
- 严格校验主名／原名、目标季、明确集数覆盖及订阅过滤器；拒绝OVA、剧场版、漫画、CD、OST、游戏等混合包。仅115目标目录真实文件确认后更新订阅。
- 需先开启“115离线下载”和DMHY开关，并保持测试模式验收；磁力熊仍在功能分支研究，未发布。

### v2.4.14

- Bot手动导入改为接收模式：`/tv`与`/movie`只选择已有剧集／电影根目录，不调用TMDB、不创建片名年份或季目录。
- 115分享只列顶层并批量转存顶层文件／文件夹，保留内部结构；离线支持无dn名称的BTIH磁力，不要求季集命名。
- 保留白名单、测试模式、互斥、协议检查、查询失败保护及成功任务哈希去重；自动追更的身份校验不变。
- 整理依赖外部监控覆盖接收根目录；不额外触发整理。顶层项目数不代表内部文件数，整目录转存可能导入大量或非视频文件，请仅提交可信分享。

### v2.4.13

- `/tv`、`/movie`自动区分115分享转存、标准ED2K文件及含dn名称的BTIH磁力离线提交，复用既有保存路径与测试模式。
- 离线资源先识别名称与类型，成功提交后保存有界哈希去重记录；提交不代表文件落盘或整理完成。
- 不支持裸磁力、BTMH-only、种子文件、任意HTTP下载。磁力实际内容无法提交前核验，离线完成须用户及外部监控确认；详见插件README限制。

### v2.4.12

- 新增 `/tv <链接>`、`/movie <链接>` Bot手动转存，复用既有剧集／电影根目录；仅单部作品，先识别与查重再转存，不修改订阅。
- 默认关闭，须填写允许的Bot用户ID；遵守测试模式，与追更及进度核验互斥，忙时提示重发。
- 剧集季集来自文件名或季目录，不能可靠识别时停止；影片多个版本仅选择一个。访问码须包含在链接参数中。
- 整理由已有外部监控处理，不另行调用，转存成功不代表整理完成。未改动Cookie方案；实机Bot路由与命令冲突需小规模验收。
