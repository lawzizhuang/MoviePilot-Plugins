...
- 离线确认与正常查重复用目标目录结果；分享递归仅重试失败目录，减少重复读取。
- 目标目录每页1000项、最多20页，逐页限速与完整性校验；读取失败或不完整时暂缓相关写入，并报告降级。
- 明确取消／过期分享使用有界六小时内存冷却，重建客户端后清空；405和超时不作为分享失效缓存。
- 不新增依赖或配置项，不修改Cookie。升级后先以测试模式及少量任务验收；尚未实现HTTP级统一限流或跨轮405冷却，不宣称根治405。

### v2.4.10

- 新增私有本地 XLSX 资源表补充源，配置容器内文件路径即可读取“分享明细”。
- 只读加载并过滤删除记录、合并重复分享；文件变化刷新内存索引，不复制表内凭据到仓库。
- Telegram 后补充115候选，夸克阶段补充夸克候选；每种网盘最多10个本地候选，复用原有身份、季集、目标目录确认及测试模式。
- 首版只导入115／夸克分享，不导入表内ED2K、磁力或其他网盘。默认关闭，升级不会主动启用或触发转存。

### v2.4.9

- **请求止损与收敛**：115 分享已取消或过期时停止逐文件重试；连续尾集追更跳过标题明确不在待补范围内的旧单集，减少无效 115 请求；离线任务提交后改用明确的待确认日志。
